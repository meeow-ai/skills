...
2. **读取评估标准**：读取 `skills/meta-skill.md`，按照其中的判断标准逐条评估候选经验是否值得沉淀。

3. **如果有值得沉淀的内容**：
   - 读取 `CLAUDE.md` 索引，检查是否已有相关 skill 文件。
   - 若已有：在原文件上更新，更新 `last_updated` 字段。
   - 若没有：按 `meta-skill.md` 的格式规范新建文件，并在 `CLAUDE.md` 索引中添加一行。
